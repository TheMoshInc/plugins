...
4b. **デザインの3点（プリセット / 画像スロット / 山場の材料＝配布物・カリキュラムの一覧）は、「おまかせ」「早く作って」と言われても必ず仮埋め＋1行確認で通す**。ここを飛ばすと配色だけ整った中身の薄い LP になる。実績数字は「確定値・集計期間・母数」が揃ったものだけ採用する（`hearing-sheet.md` #5・#8）
...
| ファイル | 内容 |
|---|---|
| `references/hearing-sheet.md` | 1往復ヒアリングシート（事前調査で仮埋め→1画面提示。必須欄＝商品・顧客像・CV先・ゴール種別（4択）、画像スロット表（収穫→依頼）、#8 見せ場の材料（配布物一覧・ロゴ・表現の禁止事項）、顧客像の合格基準と候補提示、提示フォーマット） |
